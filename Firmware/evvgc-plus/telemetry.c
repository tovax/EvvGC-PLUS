...
static void telemetryProcessCommand(const PMessage pMsg) {
  ...
  case 'I': /* Reads new mixed input settings; */
    ...
    break;
  case 'N': /* Reads new input mode settings; */
  ...
  case 'e': /* Outputs I2C error info structure; */
    ...
    pMsg->crc  = telemetryGetCRC32Checksum(pMsg);
    break;
  case 'f': /* Outputs IMU1 gyroscope data; */
  ...
  case 'i': /* Outputs input data values; */
    ...
    pMsg->crc  = telemetryGetCRC32Checksum(pMsg);
    break;
  case 'j': /* Outputs IMU1 accelerometer data; */
  ...
  case 'v': /* Outputs IMU2 accelerometer error; */
    ...
    break;
  case 'z':
    memcpy((void *)pMsg->data, (void *)&qPitch, sizeof(qPitch));
    pMsg->size = sizeof(qPitch) + TELEMETRY_MSG_SIZE;
    pMsg->crc  = telemetryGetCRC32Checksum(pMsg);
    break;
#endif /* USE_ONE_IMU */
